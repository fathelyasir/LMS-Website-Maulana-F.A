...
function AdminModule({ section, onRefresh }) {
  ...
  if (section === "tugas") {
    ...
    return <Panel title="Monitoring tugas sekolah">{tasks.map((task) => { const submissions = db.getPengumpulanForTugas(task.id); const graded = submissions.filter((item) => item.nilai != null).length; return <div className="list-row" key={task.id}><div><strong>{task.judul}</strong><small>{task.kelas} · Tenggat {formatDate(task.tenggat)}</small></div><span className="badge badge-yellow">{graded}/{submissions.length} dinilai</span></div>; })}<Empty text={!tasks.length ? "Belum ada tugas diterbitkan." : ""} /></Panel>;
  }
  if (section === "kuis") {
    const quizzes = classes.flatMap((item) => db.getKuisByKelas(item.id).map((quiz) => ({ ...quiz, kelas: item.nama })));
    return <Panel title="Monitoring kuis">{quizzes.map((quiz) => <div className="list-row" key={quiz.id}><div><strong>{quiz.judul}</strong><small>{quiz.kelas} · {quiz.soal.length} soal · {quiz.durasiMenit} menit</small></div><span className="badge badge-teal">{db.getHasilKuisByKuis(quiz.id).length} pengerjaan</span></div>)}<Empty text={!quizzes.length ? "Belum ada kuis tersedia." : ""} /></Panel>;
  }
  if (section === "forum") {
    const topics = classes.flatMap((item) => db.getForumByKelas(item.id).map((topic) => ({ ...topic, kelas: item.nama })));
    return <Panel title="Aktivitas forum">{topics.map((topic) => <div className="list-row" key={topic.id}><div><strong>{topic.judul}</strong><small>{topic.kelas} · {formatDate(topic.waktu)} · {topic.balasan.length} balasan</small><p>{topic.isi}</p></div></div>)}<Empty text={!topics.length ? "Belum ada diskusi forum." : ""} /></Panel>;
  }
  if (section === "live") {
    const lives = classes.flatMap((item) => db.getLiveClassByKelas(item.id).map((live) => ({ ...live, kelas: item.nama })));
    return <Panel title="Jadwal live class sekolah">{lives.map((live) => <div className="list-row" key={live.id}><div><strong>{live.judul}</strong><small>{live.kelas} · {formatDate(live.waktu)} · {live.hadir.length} hadir</small></div><a className="btn btn-ghost" href={live.tautan} target="_blank" rel="noreferrer">Buka</a></div>)}<Empty text={!lives.length ? "Belum ada live class terjadwal." : ""} /></Panel>;
  }
...
function AdminClasses({ classes, teachers, onRefresh }) {
  ...
  return <div className="two-column">
    <Panel title="Semua kelas dan rombel">{classes.map((item) => { const teacher = teachers.find((user) => user.id === item.guruId); return <div className="list-row" key={item.id}><div><strong>{item.nama} · {item.mapel}</strong><small>Kode {item.kodeKelas} · Pengajar: {teacher?.nama || "Belum ditentukan"} · {item.siswaIds.length} siswa</small></div><span className="badge badge-teal">Aktif</span></div>; })}<Empty text={!classes.length ? "Belum ada kelas yang terdaftar." : ""} /></Panel>
